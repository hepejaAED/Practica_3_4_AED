...
```{r}
pew_religion_tidy_2<-gather(pew_religion,income,freq,2:11) %>% mutate(income=as.factor(income))
str(pew_religion_tidy_2)
```
...
```{r}
library(lubridate)
raw %>% pivot_longer(cols=starts_with("wk"),names_prefix = "wk",names_to = "week",values_to="rank")
# %>%mutate(date=date.entered+weeks(as.numeric(week)-1))
...
```{r}
raw %>%gather(sex,value=cases,3:ncol(raw)) %>% separate(sep=1,col=sex,c("sex","age")) 
# Falta ver cómo poner 0-14
```
...
```{r}
raw %>% pivot_longer(cols=starts_with("d"),names_to="day",names_prefix = "d",values_to = "value") %>% drop_na() %>% pivot_wider(names_from ="element",values_from="value") %>% unite(date,year,month,day,sep="-") %>% mutate(date=as_datetime(date)) 

# pivot_longer(cols=starts_with("wk"),names_prefix = "wk",names_to = "week",values_to="rank")
```
...
```{r }
planes.f<-planes %>% filter(!str_detect(str_to_lower(manufacturer),".*douglas.*")) %>%
  filter(between(seats,90,210)) %>% mutate(manufacturer=ifelse(str_detect(str_to_lower(manufacturer),".*airbus*"),"AIRBUS COMP",manufacturer))
...
flights.f<-flights %>% inner_join(planes.f %>% select(tailnum),by="tailnum") 
...
# flights<-flights %>% select(-time_hour,-starts_with("time"))
...
flights<-flights %>% unite(Full_Date,day,month,year,sep="-") %>% mutate(Full_Date=as.Date(Full_Date))
...
agregado<-flights %>% group_by(tailnum) %>% summarise(Num_flights=n(),Mean_distance=mean(distance), Total_distance=sum(distance)) %>% drop_na()
...
df<-raw %>% pivot_longer(cols=starts_with("wk"),names_prefix = "wk",names_to = "week",values_to="rank") %>%  drop_na(rank)

songs<-df %>% select(artist,track,time) %>% mutate(id=row_number())
ranking<-df %>% select(date.entered,week,rank)
```
...
```{r}
library(tidyverse)
table5 %>% unite("year",century,year,sep="") %>% separate(sep="/",rate,into=c("cases","population")) %>%   mutate(
